...
def plot_all(df):
	...
	# Volumes
	date_store = df[["numbot", "storenum", "date"]].\
	...
	fig = px.histogram(df, x="lnprice").\
	update_layout(xaxis_title = "Log Price", yaxis_title = "Count", title = "Two broad price categories")
	
	fig.write_image("figures/log_price_hist.png")
	df["price_cat"] = pd.cut(df.lnprice, bins = 2, labels = ["Popular", "Super premium"])
